...
type BrainData = {
  ...
  leftLineGeo: THREE.BufferGeometry;
  rightLineGeo: THREE.BufferGeometry;
  crossLineGeo: THREE.BufferGeometry;
};
...
function generateBrain(): BrainData {
  ...
  const colorB = new THREE.Color("#c084fc");
  const leftPts = pts.filter((p) => p.x <= 0);
  const rightPts = pts.filter((p) => p.x > 0);

  const buildLines = (points: THREE.Vector3[]) => {
  ...
  for (let i = 0; i < leftPts.length; i++) {
    for (let j = 0; j < rightPts.length; j++) {
      ...
      if (dist < 0.5 && Math.abs(leftPts[i].x) < 0.25 && Math.abs(rightPts[j].x) < 0.25) {
        ...
        const color = rand() > 0.5 ? colorA : colorB;
        crossColors.push(color.r, color.g, color.b, color.r, color.g, color.b);
      }
    }
  }

  const crossLineGeo = new THREE.BufferGeometry();
  crossLineGeo.setAttribute("position", new THREE.Float32BufferAttribute(crossPositions, 3));
  crossLineGeo.setAttribute("color", new THREE.Float32BufferAttribute(crossColors, 3));

  return {
    leftPts,
    rightPts,
    leftLineGeo: buildLines(leftPts),
    rightLineGeo: buildLines(rightPts),
    crossLineGeo,
  };
}
        ...
        >
          <div
            ...
            style={{
              ...
              boxShadow:
                "0 8px 32px rgba(0,0,0,0.45), inset 0 0 16px rgba(96, 165, 250, 0.08)",
...
function SplitBrain({ progressRef }: { progressRef: { current: number } }) {
  ...
  const memoryNodes = useMemo(() => {
    ...
    const shuffled = [...allPts].sort((a, b) => {
      const scoreA = Math.sin(a.x * 12.9898 + a.y * 78.233 + a.z * 37.719);
      const scoreB = Math.sin(b.x * 12.9898 + b.y * 78.233 + b.z * 37.719);
      return scoreA - scoreB;
    });

    for (const p of shuffled) {
      const tooClose = selected.some((sp) => p.distanceTo(sp) < 1.0);
  ...
  useFrame((state) => {
    ...
    const split = easedProgress * (isMobile ? 4.2 : 5);

    if (leftRef.current) {
      leftRef.current.position.x = -split;
      leftRef.current.rotation.y = -easedProgress * 0.5;
    }

    if (rightRef.current) {
      rightRef.current.position.x = split;
      rightRef.current.rotation.y = easedProgress * 0.5;
    }
    ...
    if (crossLineMatRef.current) {
      crossLineMatRef.current.opacity = baseOpacity * Math.max(0, 1 - easedProgress * 4);
    }
    if (coreMatRef.current) {
      coreMatRef.current.opacity = 0.03 * (1 - progress);
    }
  });
  ...
  return (
    <group ref={groupRef} scale={brainScale}>
      ...
      <mesh>
        ...
        <meshBasicMaterial ref={coreMatRef} color="#60a5fa" transparent opacity={0.03} />
      </mesh>
...
export function BrainOverlay() {
  ...
  const styleSnapshotRef = useRef<StyleSnapshot | null>(null);
  const doneRef = useRef(false);
  const [mounted, setMounted] = useState(true);

  const textOpacity = useTransform(progress, [0.12, 0.28, 0.62, 0.78], [0, 1, 1, 0]);
  const textScale = useTransform(progress, [0.12, 0.36], [0.92, 1]);
  const overlayOpacity = useTransform(progress, [0.76, 1], [1, 0]);
  ...
  useEffect(() => {
    ...
    const step = () => {
      ...
      const next = current + (targetProgress - current) * 0.24;
      const settled = Math.abs(targetProgress - next) < 0.003 ? targetProgress : next;
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      const pixels = e.deltaY * unit;
      const capped = Math.sign(pixels) * Math.min(Math.abs(pixels), 180);
      update(capped * 0.0018);
    };
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      touchStartY = e.touches[0].clientY;
      const capped = Math.sign(delta) * Math.min(Math.abs(delta), 120);
      update(capped * 0.006);
    };
  ...
  return (
    ...
    >
      <div style={{ position: "absolute", inset: 0 }}>
        ...
        >
          <SplitBrain progressRef={progressRef} />
