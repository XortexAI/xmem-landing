  ...
  return (
    <div className="group relative overflow-hidden rounded-md border border-white/10 bg-[#050505]">
      <button
        ...
        title="Copy code"
      >
        {copied ? <CheckCircle2 className="h-4 w-4 text-[#b8ff65]" /> : <Clipboard className="h-4 w-4" />}
      </button>
      ...
      <main className="relative z-10 mx-auto grid max-w-7xl gap-8 px-5 pb-24 pt-28 sm:px-8 lg:grid-cols-[260px_minmax(0,1fr)_220px] lg:pt-32">
        <aside className="hidden lg:block">
          <div className="sticky top-28 rounded-md border border-white/10 bg-black/35 p-4 backdrop-blur-xl">
            <Link href="/docs" className="mb-4 flex items-center gap-2 rounded-md bg-white/[0.04] px-3 py-2 text-sm font-semibold text-white">
        ...
        <article className="min-w-0">
          ...
          {page.slug === "overview" && (
            <div className="my-8 grid gap-3 md:grid-cols-3">
              {[
                ["Memory writes", "Judge-approved add, update, delete, and skip operations.", Database],
                ["Agent clients", "MCP and npm connectors for coding assistants.", Cpu],
                ["Code context", "Scanner, graph memory, and repository-aware retrieval.", GitBranch],
              ].map(([title, text, CardIcon]) => (
                <div key={title as string} className="rounded-md border border-white/10 bg-white/[0.03] p-5">
                  <CardIcon className="mb-4 h-5 w-5 text-[#b8ff65]" />
        ...
        <aside className="hidden xl:block">
          <div className="sticky top-28 space-y-5">
            <div className="rounded-md border border-white/10 bg-black/35 p-4">
              <div className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em] text-white/35">
              ...
              <nav className="space-y-1">
                {page.sections.map((section) => (
                  <a
                    key={section.id}
                    href={`#${section.id}`}
                    className="flex items-center gap-2 rounded-md px-2 py-1.5 text-sm text-white/45 transition hover:bg-white/[0.05] hover:text-white"
                  >
            ...
            <div className="rounded-md border border-white/10 bg-white/[0.03] p-4">
              <div className="mb-2 flex items-center gap-2 text-sm font-semibold text-white">
                <LockKeyhole className="h-4 w-4 text-[#b8ff65]" />
                Secure by default
              </div>
